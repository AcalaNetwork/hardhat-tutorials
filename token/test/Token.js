    ...
    describe('Transfer', function () {
      describe('transfer()', function () {
        ...
        it('should emit a Transfer event when transfering the token', async function () {
          await expect(instance.connect(deployer).transfer(userAddress, 100))
            .to.emit(instance, 'Transfer')
            .withArgs(deployerAddress, userAddress, 100);
        });

        it('should revert the transfer to a 0x0 address', async function () {
          await expect(instance.connect(deployer).transfer(NULL_ADDRESS, 100)).to.be.revertedWith(
            'ERC20: transfer to the zero address'
          );
        });

        it('should revert if trying to transfer amount bigger than balance', async function () {
          await expect(instance.connect(deployer).transfer(userAddress, 12345678900)).to.be.revertedWith(
    ...
    describe('Allowances', function () {
      describe('approve()', function () {
        ...
        it('should emit Approval event when calling approve()', async function () {
          await expect(instance.connect(deployer).approve(userAddress, 100))
            .to.emit(instance, 'Approval')
            .withArgs(deployerAddress, userAddress, 100);
        });

        it('should revert when trying to give allowance to 0x0 address', async function () {
          await expect(instance.connect(deployer).approve(NULL_ADDRESS, 100)).to.be.revertedWith(
      ...
      describe('increaseAllowance()', function () {
        ...
        it('should emit Approval event', async function () {
          await instance.connect(deployer).approve(userAddress, 100);
          await expect(instance.connect(deployer).increaseAllowance(userAddress, 40))
            .to.emit(instance, 'Approval')
      ...
      describe('decreaseAllowance()', function () {
        ...
        it('should emit Approval event', async function () {
          await instance.connect(deployer).approve(userAddress, 100);
          await expect(instance.connect(deployer).decreaseAllowance(userAddress, 40))
            .to.emit(instance, 'Approval')
        ...
        it('should revert when tyring to decrease the allowance below 0', async function () {
          await instance.connect(deployer).approve(userAddress, 100);
          await expect(instance.connect(deployer).decreaseAllowance(userAddress, 1000)).to.be.revertedWith(
